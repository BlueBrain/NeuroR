...
from neuror.zero_diameter_fixer import (
    _fix_downstream,
    _fix_in_between,
    _fix_upstream,
    fix_zero_diameters,
...
def test_fix_in_between():
    ...
    assert_array_equal(root.diameters, np.array([2.0, 2.0, 2.2, 2.4, 2.8, 3.0], dtype=np.float32))
    assert_array_equal(root.children[0].diameters, np.array([3, 3, 3, 2.8], dtype=np.float32))
    assert_array_equal(
        root.children[0].children[0].diameters,
        np.array([2.8, 2.6, 2.4, 2.2, 2.0], dtype=np.float32),
    )
...
def test_fix_in_between_legacy():
    ...
    assert_array_equal(root.diameters, np.array([2.0, 2.0, 2.25, 2.5, 2.75, 3.0], dtype=np.float32))
    assert_array_almost_equal(
        root.children[0].diameters,
        np.array([3.0, 3.0, 3.0, 2.833333], dtype=np.float32),
    )
    assert_array_almost_equal(
        root.children[0].children[0].diameters,
        np.array([2.666667, 2.5, 2.333333, 2.166667, 2.0], dtype=np.float32),
    )
...
def test_fix_from_downstream():
    ...
    assert_array_equal(
        neuron.section(0).diameters,
        np.array([2.0, 2.0, 1.0e-6, 1.0e-6, 1.0e-6, 3.0], dtype=np.float32),
    ...
    assert_array_equal(root.diameters, [0.0, 0.0])
    _fix_downstream(root)
    assert_array_equal(
        root.diameters,
        [0.0, 0.0],
        "Diameters should not have been changed " "since the whole neurite has a null diameter",
    )
