...
def test_cut_plane_from_rotations_translations():
    ...
    assert_array_almost_equal(equation.coefs, [35.836795, 0.0, 93.358043, -4667.902132])


def test_cut_neuron_simple():
    filename = DATA / "simple2.asc"
    result = CutPlane.find(filename, bin_width=0.2).to_json()
    assert "The probability that there is in fact NO cut plane is high: -log(p)" in result["status"]
...
def test_cut_real_neuron():
    ...
    assert_almost_equal(result["cut-plane"]["b"], 0)
    assert_almost_equal(result["cut-plane"]["c"], 1)
    assert_almost_equal(result["cut-plane"]["d"], -48.68020515427703, decimal=5)
    assert result["cut-plane"]["comment"] == "Equation: a*X + b*Y + c*Z + d < 0"

    leaves_coord = [
        [63.97577896, 61.52564626, 44.46020393],
