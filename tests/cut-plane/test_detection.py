...
def test_project_normal():
    ...
    assert_array_equal(plane.project_on_normal(np.array([[0, 0, 2], [1, 2, 3]])), [2, 3])

    plane = PlaneEquation(0, 1, 0, -9)
    assert_array_equal(plane.project_on_normal(np.array([[0, 0, 0], [1, 2, 3]])), [-9, -7])
...
@pytest.mark.skipif(
    version.parse(scipy.__version__) >= version.parse("1.7"),
...
def test_minimize_more17():
    ...
    assert_allclose(
        result,
        [4.1510e00, 4.6763e01, -2.0966e01, -2.7207e-04, -4.5070e-04, 6.8804e01],
        rtol=1e-4,
    )
...
def test_plane_equation():
    ...
    assert_array_equal(equation.distance([[2, 3, 4], [2, 3, 7]]), [1, 2])
...
def test_from_json():
    ...
    def _assert_expected_plane(plane):
        ...
        assert_array_equal(plane.cut_leaves_coordinates, [[10, 10, 10], [1, 0, 20]])
        assert plane.status == "ok"
        assert plane.minus_log_prob == 70

    _assert_expected_plane(test_module.CutPlane.from_json(str(filename)))

    _assert_expected_plane(test_module.CutPlane.from_json(filename))

    with filename.open() as file_:
        dictionnary = json.load(file_)
        _assert_expected_plane(test_module.CutPlane.from_json(dictionnary))
...
def test_find():
    ...
    assert set(result.keys()) == {"details", "cut-plane", "cut-leaves", "status"}
