...
def test_legacy():
    ...
    assert_array_equal(points, [[6.0, -4.0, 0.0], [-5.0, -4.0, 0.0]])
    assert sign == -1

    with pytest.raises(Exception):
        test_module.cut_detect(SIMPLE, "z")
...
def test_get_obliques():
    ...
    assert [sec.id + OFFSET for sec in test_module.get_obliques(neuron, extended_types)] == [217]
...
def test_children_ids():
    ...
    assert [sec.id + OFFSET for sec in test_module.children_ids(section)] == [
        134,
        135,
        136,
        137,
...
def test_legacy_compare_with_legacy_result():
    ...
    cut_sections = {point_to_section_segment(neuron, point)[0] + 1 for point in points}
