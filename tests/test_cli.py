...
def test_error_annotation_folder(tmpdir):
    result = _run(["error-annotation", "folder", str(DATA / "test-error-detection"), str(tmpdir)])
    assert result.exit_code == 0, result.exception
...
def test_repair_folder(tmpdir):
    result = _run(["cut-plane", "repair", "folder", str(DATA / "input-repair-all"), str(tmpdir)])
    assert result.exit_code == 0, result.exception
    assert set(str(p.relative_to(tmpdir)) for p in Path(tmpdir).rglob("*")) == {
        "simple.asc",
        "simple2.asc",
...
def test_sanitize(tmpdir):
    ...
    result = _run(["sanitize", "folder", str(DATA), str(tmpdir)])
    assert result.exit_code == 0
