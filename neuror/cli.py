...
@click.option(
    ...
    default=None,
    help=(
        "Path to a CSV whose columns represents the X, Y and Z "
...
@click.option(
    ...
    default=None,
    help=(
        "A dir with the cut points CSV file for each morphology. "
...
@click.option(
    ...
    type=click.Path(file_okay=True),
    default=None,
    help=("Path to the file that contains the coordinate mapping before " "and after unravelling"),
)
...
def _export_cut_plane(filename, output, width, display, searched_axes, fix_position):
    ...
    result = CutPlane.find(
        filename, width, searched_axes=searched_axes, fix_position=fix_position
    ).to_json()
...
@click.argument("filename", type=str, required=True)
@click.option("-o", "--output", help=("Output name for the JSON file (default=STDOUT)"))
@click.option(
    "-w",
    "--width",
    type=float,
