...
def _unravel_section(section, window_half_length, soma, legacy_behavior):
    ...
    for window_center in range(1, point_count):
        ...
        direction *= np.sign(scalar_product or 1.0)
...
def unravel(filename, window_half_length=None, legacy_behavior=False, use_path_length=True):
    ...
    if window_half_length is None:
        window_half_length = (
            DEFAULT_WINDOW_HALF_LENGTH_PATH if use_path_length else DEFAULT_WINDOW_HALF_LENGTH
        )
    ...
    for sec, new_section in zip(morph.iter(), new_morph.iter()):
        ...
        if legacy_behavior and len(morph.soma.points) > 1:  # pylint: disable=no-member
            coord_before = np.vstack(
                (morph.soma.points[0], coord_before)
            )  # noqa, pylint: disable=no-member
...
def unravel_plane(plane, mapping):
    ...
    if not_matching_leaves.size:
        raise Exception(
            "Cannot find the following leaves in the mapping:\n" + str(leaves[not_matching_leaves])
        )
...
def unravel_all(
    raw_dir, unravelled_dir, raw_planes_dir, unravelled_planes_dir, window_half_length=None
):
    ...
    for inputfilename in iter_morphology_files(raw_dir):
        ...
        try:
            ...
            L.warning("Unravelling %s failed", inputfilename)
