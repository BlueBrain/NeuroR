...
def create_plane(pos, quat):
    ...
    length = np.linalg.norm(BBOX[1] - BBOX[0]) / 2.0
                    ...
                    [
                        ...
                        html.Div(
                            [
                                ...
                                dcc.Checklist(
                                    id="hide-plane",
                                    options=[{"label": "Hide plane", "value": "hidden"}],
                                    value=[],
                                ),
...
def display_click_data(rot_x, rot_y, rot_z, transl_x, transl_y, transl_z, hide, layout):
    ...
    qx = Quaternion(axis=[1, 0, 0], angle=rot_x / 180.0 * np.pi)
    qy = Quaternion(axis=[0, 1, 0], angle=rot_y / 180.0 * np.pi)
    qz = Quaternion(axis=[0, 0, 1], angle=rot_z / 180.0 * np.pi)
...
def update_output(rot_x, rot_y, rot_z, transl_x, transl_y, transl_z):
    ...
    return {
        ...
        "layout": {
            ...
            "xaxis": {"title": "Distance to plane (um)"},
            "yaxis": {"title": "Counts"},
            "shapes": [
                # Line Vertical
                {
...
@app.callback(Output("rotate-x-slider", "value"), [Input("optimized", "data-*")])
def update_post_optim_x_rotate(params):
    """callback"""
    return params[0]


@app.callback(Output("rotate-y-slider", "value"), [Input("optimized", "data-*")])
def update_post_optim_y_rotate(params):
    """callback"""
    return params[1]


@app.callback(Output("rotate-z-slider", "value"), [Input("optimized", "data-*")])
def update_post_optim_z_rotate(params):
    """callback"""
    return params[2]


@app.callback(Output("translate-x-slider", "value"), [Input("optimized", "data-*")])
def update_post_optim_x_translate(params):
    """callback"""
    return params[3]


@app.callback(Output("translate-y-slider", "value"), [Input("optimized", "data-*")])
def update_post_optim_y_translate(params):
    """callback"""
    return params[4]


@app.callback(Output("translate-z-slider", "value"), [Input("optimized", "data-*")])
...
def export(n_clicks, rot_x, rot_y, rot_z, transl_x, transl_y, transl_z, output_path):
    ...
    plane = CutPlane.from_rotations_translations(
        [rot_x, rot_y, rot_z, transl_x, transl_y, transl_z], NEURON, BIN_WIDTH
    )
