...
class PlaneEquation(object):
    ...
    def from_rotations_translations(cls, transformations):
        ...
        qx = Quaternion(axis=[1, 0, 0], angle=rot_x / 180.0 * np.pi).unit
        qy = Quaternion(axis=[0, 1, 0], angle=rot_y / 180.0 * np.pi).unit
        qz = Quaternion(axis=[0, 0, 1], angle=rot_z / 180.0 * np.pi).unit
        quat = qx * qy * qz
